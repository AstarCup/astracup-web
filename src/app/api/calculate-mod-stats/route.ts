...
// 保存.osu文件内容到临时文件
function saveOsuFileToTemp(
  osuContent: string,
  filename: string = "temp.osu",
): string {
    ...
    try {
      ...
      child.on("close", (code) => {
        ...
        if (code !== 0) {
          ...
          reject(
            new Error(`OsuNodeHelper failed: ${stderr || "Unknown error"}`),
          );
          return;
        }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { customModName, customDASettings, customDTRate } =
      customSettings || {};
    ...
    if (!beatmapId && !osuContent) {
      return NextResponse.json(
        { error: "Either beatmapId or osuContent is required" },
        { status: 400 },
      );
    }
    ...
    if (mods && mods !== "NM") {
      ...
      if (modString === "LZ" && customModName) {
        ...
        // 处理其他已知的Lazer特有mod
          ...
          ["WG", "MR", "RD", "AS", "CL", "SG", "TC", "AC"].includes(customMod)
        ) {
          // 这些mod大多是视觉或转换效果，不影响难度计算
          // 但我们记录它们以便将来扩展
          console.log(
            `Lazer mod detected: ${customMod} (visual/conversion effect)`,
          );
        }
        // 其他未知的lazer mod
        else {
    ...
    try {
      // 调用可执行文件计算难度
      const result = await calculateDifficultyWithExe(
        beatmapId,
        modsArray,
        modOptions,
        accessToken,
