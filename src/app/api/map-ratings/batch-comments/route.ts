...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      "Batch comments API called with mapSelectionIds:",
    ...
    // 不限制批量请求的数量，支持获取所有评论
    console.log(
      `Processing batch request for ${mapSelectionIds.length} map selections`,
    );
    ...
    console.log(
      "Batch comments result count:",
      Object.keys(batchComments).length,
    );
    ...
    console.log(
      "Returning response with comments for",
      Object.keys(batchComments).length,
      "map selections",
    );
