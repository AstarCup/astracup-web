...
export async function POST(request: NextRequest) {
    try {
        ...
        // 更新消息状态
        const success = await updateMessageStatus(messageId, status, response_action);
        ...
        if (success) {
            ...
            if (action === 'accept' || action === 'decline') {
                ...
                if (message && message.related_matchup_id) {
                    // 根据响应更新对战状态
                    const newMatchupStatus = action === 'accept' ? 'scheduled' : 'available';
                    await updatePlayerMatchupStatus(message.related_matchup_id, newMatchupStatus);
                }
            }
