...
export async function GET(
    ...
    try {
        ...
        if (!scoresResponse.ok) {
            console.error(`Failed to fetch scores for room ${roomId}, playlist ${playlistId}:`, scoresResponse.status, scoresResponse.statusText);
            return NextResponse.json(
        ...
        console.log(`Room ${roomId}, playlist ${playlistId}: fetched ${scoresData.scores?.length || 0} scores`);

        // 转换数据格式
        const displayScores: DisplayScore[] = scoresData.scores?.map((score: any, index: number) => ({
