...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `开始获取 ${roomIds.length} 个房间的分数数据，优先从数据库获取...`,
    );
      ...
      try {
        ...
        if (dbResponse.ok) {
          ...
          if (dbData.success && dbData.scores && dbData.scores.length > 0) {
            ...
            console.log(
              `从数据库获取到房间 ${roomId} 的 ${dbData.scores.length} 条分数`,
            );
            ...
            const roomInfo = {
              ...
              playlist_count:
                dbData.scores.length > 0
                  ? new Set(dbData.scores.map((s: any) => s.playlistId)).size
                  : 0,
            };
            roomsData.push(roomInfo);

            // 处理数据库分数数据，确保字段名正确
            const processedScores: DisplayScore[] = dbData.scores
              .map((score: any) => {
                // 验证玩家信息的完整性
                if (!score.user_id || !score.username) {
                  console.warn(
                    `[Player Info Error] Invalid player info in score from database room ${roomId}: user_id=${score.user_id}, username=${score.username}`,
                  );
                  return null;
                }

                console.log(
                  `[Player Info] Processing score for user ${score.username} (ID: ${score.user_id}) from room ${roomId}`,
                );
                ...
                const processedScore: DisplayScore = {
                  ...
                  beatmapset_id:
                    score.beatmapset_id || (score as any).beatmapsetId,
                  // 添加房间信息
                  roomId: roomId,
        ...
        for (const playlistItem of room.playlist) {
          try {
            ...
            if (playlistItemResponse.ok) {
              ...
              playlistItemBeatmapsetId =
                playlistItemData.beatmap?.beatmapset_id;
            } else {
            ...
            if (!scoresResponse.ok) {
              console.error(
                `Failed to fetch scores for room ${roomId}, playlist ${playlistItem.id}:`,
            ...
            const scoresData: MultiplayerScoresResponse =
              await scoresResponse.json();
            console.log(
              `房间 ${roomId} 图池 ${playlistItem.id} 获取到 ${scoresData.scores.length} 条分数`,
            );
            ...
            console.error(
              `Error processing playlist ${playlistItem.id} for room ${roomId}:`,
              error,
            );
            continue;
          }
