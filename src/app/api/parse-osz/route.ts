...
export async function POST(request: NextRequest) {
  try {
    ...
    if (parsedInBrowser && browserBeatmapInfos.length > 0) {
      ...
      for (const beatmapInfo of browserBeatmapInfos) {
        try {
          if (beatmapInfo.osuContent) {
            ...
            const modStatsResponse = await fetch(
              `${request.nextUrl.origin}/api/calculate-mod-stats`,
              {
                method: "POST",
                headers: {
          ...
          console.error(
            `Error calculating mod stats for ${beatmapInfo.osuFilename}:`,
            error,
          );
          // 出错时使用原始信息
          beatmapInfosWithModStats.push({
      ...
      // 按难度名排序
      beatmapInfosWithModStats.sort((a, b) =>
    ...
    try {
      ...
      for (const beatmapInfo of beatmapInfos) {
        try {
          // 获取.osu文件内容
          const osuContent = await zip
            .file(beatmapInfo.osuFilename)
            ?.async("text");
          if (osuContent) {
            // 调用calculate-mod-stats API计算mod后的属性
            const modStatsResponse = await fetch(
