...
const MOD_OPTIONS = ["NM", "HD", "HR", "DT", "FM", "LZ", "TB"];
...
export default function MapSelectionManagement({
  user,
  permissions,
}: MapSelectionManagementProps) {
  ...
  const [commentsByMapSelectionId, setCommentsByMapSelectionId] = useState<{
    [key: number]: any[];
  }>({});
  ...
  const [beatmapPreview, setBeatmapPreview] = useState<BeatmapInfo | null>(
    null,
  );
  ...
  const [customPoolType, setCustomPoolType] = useState<"original" | "custom">(
    "original",
  );
  ...
  const [availableLazerMods, setAvailableLazerMods] = useState<
  ...
  // 批量过审相关状态
  const [tempApprovedSelections, setTempApprovedSelections] = useState<
    Set<number>
  >(new Set());
  ...
  const fetchSelections = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/map-selections?season=${season}&category=${category}&osuId=${userForState.id}`,
      );
      if (response.ok) {
  ...
  const calculateModdedStatsAPI = useCallback(
    async (
      beatmap: BeatmapInfo,
      mods: string,
  ...
  const fetchBatchComments = useCallback(async (mapSelectionIds: number[]) => {
    ...
    try {
      const response = await fetch(
        `/api/map-ratings/batch-comments?mapSelectionIds=${mapSelectionIds.join(",")}`,
      );
      if (response.ok) {
        const data = await response.json();
        if (data.success) {
    ...
    const updateModdedStats = async () => {
      if (beatmapPreview && user) {
        ...
        const customSettings = {
          ...
          customDASettings:
            customModName === "DA" && modForCalculation === "LZ"
              ? {
              ...
              : null,
          customDTRate:
            modForCalculation === "DT" && customDTRate !== ""
              ? customDTRate
              : null,
        };
        const stats = await calculateModdedStatsAPI(
          beatmapPreview,
          modForCalculation,
          customSettings,
        );
      ...
      const response = await fetch("/api/map-selections", {
        ...
        body: JSON.stringify({
          ...
          totalLength:
            modForCalculation === "DT" && customDTRate !== ""
              ? Math.round(
                  beatmapPreview.total_length / (customDTRate as number),
                )
      ...
      for (const osuFile of osuFiles) {
        try {
          ...
          beatmapInfos.push({
            ...
            titleUnicode:
              parsedData["TitleUnicode"] || parsedData["Title"] || "",
            artist: parsedData["Artist"] || "",
            artistUnicode:
              parsedData["ArtistUnicode"] || parsedData["Artist"] || "",
  ...
  const fillFormWithBeatmapInfo = (beatmapInfo: any) => {
    ...
    setCustomPoolCS(
      beatmapInfo.cs ? parseFloat(beatmapInfo.cs.toFixed(2)) : "",
    );
    setCustomPoolAR(
      beatmapInfo.ar ? parseFloat(beatmapInfo.ar.toFixed(2)) : "",
    );
    setCustomPoolOD(
      beatmapInfo.od ? parseFloat(beatmapInfo.od.toFixed(2)) : "",
    );
    setCustomPoolHP(
      beatmapInfo.hp ? parseFloat(beatmapInfo.hp.toFixed(2)) : "",
    );
    setCustomBPM(beatmapInfo.bpm ? parseFloat(beatmapInfo.bpm.toFixed(2)) : "");
    const totalLengthInSeconds = beatmapInfo.totalLength
      ? Math.round(beatmapInfo.totalLength / 1000)
      : "";
    setCustomTotalLength(totalLengthInSeconds);
    setCustomStarRating(
      beatmapInfo.starRating
        ? parseFloat(beatmapInfo.starRating.toFixed(2))
        : "",
    );
  ...
  useEffect(() => {
    const recalculateOnModChange = async () => {
      if (
        oszBeatmapInfos.length > 0 &&
        selectedDifficultyIndex >= 0 &&
        selectedDifficultyIndex < oszBeatmapInfos.length
      ) {
        ...
        if (beatmapInfo && beatmapInfo.osuContent) {
          try {
            ...
            if (response.ok) {
              ...
              setCustomPoolCS(
                modStats.cs ? parseFloat(modStats.cs.toFixed(2)) : "",
              );
              setCustomPoolAR(
                modStats.ar ? parseFloat(modStats.ar.toFixed(2)) : "",
              );
              setCustomPoolOD(
                modStats.od ? parseFloat(modStats.od.toFixed(2)) : "",
              );
              setCustomPoolHP(
                modStats.hp ? parseFloat(modStats.hp.toFixed(2)) : "",
              );
              setCustomBPM(
                modStats.bpm ? parseFloat(modStats.bpm.toFixed(2)) : "",
              );
              const totalLengthInSeconds = modStats.totalLength
                ? Math.round(modStats.totalLength / 1000)
                : "";
              setCustomTotalLength(totalLengthInSeconds);
              setCustomStarRating(
                modStats.starRating
                  ? parseFloat(modStats.starRating.toFixed(2))
                  : "",
              );
            ...
            console.error(
              "Error recalculating mod stats on mod change:",
              error,
            );
          }
        }
  ...
  const addCustomPool = async () => {
    ...
    try {
      ...
      // 根据选择设置customModName
      const customModNameValue =
        customPoolType === "original" ? "原创" : "定制";
      ...
      if (oszFile && oszUploadSuccess) {
        try {
          ...
          if (!uploadData.success) {
            console.warn(
              "重新上传osz文件失败，但继续提交自定图池:",
              uploadData.error,
            );
          }
        } catch (uploadError) {
          console.warn(
            "重新上传osz文件时出错，但继续提交自定图池:",
            uploadError,
          );
        }
      }
  ...
  const deleteSelection = async (selectionId: number) => {
    try {
      const response = await fetch(
        `/api/map-selections?id=${selectionId}&selectedBy=${userForState.id}`,
        {
          method: "DELETE",
        },
      );
  ...
  const refreshSelection = async (selection: MapSelection) => {
    try {
      ...
      if (!beatmapData.success || !beatmapData.data) {
        showError(
          "重新获取beatmap数据失败: " + (beatmapData.error || "未知错误"),
        );
        return;
      }
      ...
      // 获取最新的beatmap信息
      const latestBeatmap =
        beatmapData.data.type === "single"
          ? beatmapData.data.beatmap
          : Array.isArray(beatmapData.data.beatmaps)
            ? beatmapData.data.beatmaps.find(
                (b: BeatmapInfo) => b.id === selection.beatmapId,
              )
            : null;
        ...
        prev.map((s) =>
          s.id === selection.id
            ? {
                ...
                // 计算DT时长
                totalLength:
                  selection.selectedMods === "DT" && selection.customDTRate
                    ? Math.round(
                        latestBeatmap.total_length / selection.customDTRate,
                      )
                    : latestBeatmap.total_length,
                // 同时更新基础beatmap信息（包括Unicode字段）
                title: latestBeatmap.title,
                title_unicode:
                  latestBeatmap.title_unicode || latestBeatmap.title,
                artist: latestBeatmap.artist,
                artist_unicode:
                  latestBeatmap.artist_unicode || latestBeatmap.artist,
  ...
  // 更新选图属性
  const updateSelectionAttributes = async (
    selectionId: number,
    updates: {
      title?: string;
      version?: string;
  ...
  const handleBulkApproval = async () => {
    ...
    try {
      ...
      if (data.success) {
        showSuccess(
          `批量过审成功，共过审 ${tempApprovedSelections.size} 个选图`,
        );
  ...
  const getModColorBorderClass = (mod: string): string => {
    switch (mod) {
      case "NM":
        return "border-b-4 border-b-blue-500 text-blue-500";
      case "HD":
        return "border-b-4 border-b-yellow-500 text-yellow-500";
      case "HR":
        return "border-b-4 border-b-red-500 text-red-500";
      case "DT":
        return "border-b-4 border-b-purple-500 text-purple-500";
      case "EZ":
        return "border-b-4 border-b-green-500 text-green-500";
      case "LZ":
        return "border-b-4 border-b-pink-600 text-pink-600";
      case "TB":
        return "border-b-4 border-b-black text-black";
      case "FM":
        return "border-b-4 border-b-blue-500 text-blue-500";
      default:
        return "border-b-4 border-b-blue-500 text-blue-500";
  ...
  const parseOsuUrl = (
    url: string,
  ): { beatmapId?: number; beatmapsetId?: number } | null => {
    try {
      // 处理完整的 URL 或缩短的 URL
      ...
      const beatmapsetWithMapMatch = cleanUrl.match(
        /^\/beatmapsets\/(\d+)#osu\/(\d+)$/,
      );
    ...
    .filter((selection) => {
      ...
      if (searchQuery.trim()) {
        ...
        if (/^\d+$/.test(query)) {
          ...
          return (
            selection.beatmapId === numericQuery ||
            selection.beatmapsetId === numericQuery
          );
        }
        ...
        if (urlData) {
          if (urlData.beatmapId && urlData.beatmapsetId) {
            ...
            return (
              selection.beatmapId === urlData.beatmapId &&
              selection.beatmapsetId === urlData.beatmapsetId
            );
        ...
        const simplifiedPropertyMatch = query.match(
          /^([a-zA-Z]+)(\d+(?:\.\d+)?)$/,
        );
        if (simplifiedPropertyMatch) {
          ...
          switch (property.toLowerCase()) {
            ...
            case "tb":
              return (
                selection.selectedMods.toLowerCase() ===
                  property.toLowerCase() && selection.modPosition === value
              );
            default:
              break;
          }
        ...
        if (specialSearchMatch) {
          ...
          switch (key.toLowerCase()) {
            ...
            case "mod":
              return (
                selection.selectedMods.toLowerCase() === value.toLowerCase()
              );
            default:
              // 如果不是特殊格式，回退到普通文本搜索
              break;
        ...
        return (
          ...
          (selection.title_unicode &&
            selection.title_unicode.toLowerCase().includes(query)) ||
          selection.artist.toLowerCase().includes(query) ||
          (selection.artist_unicode &&
            selection.artist_unicode.toLowerCase().includes(query)) ||
          ...
          (selection.selectedByUsername &&
            selection.selectedByUsername.toLowerCase().includes(query))
        );
      }
  ...
  const getModFilterOptions = () => {
    ...
    // 生成选项列表
    const options = [{ value: "all", label: "全部", count: selections.length }];
    ...
    <div className="max-w-9xl mx-auto p-6">
      ...
      <div className="mb-4 flex gap-4 flex-wrap">
        ...
        {(permissions.isadmin) && (
          <button
            onClick={() => {
              // 获取当前筛选出的未过审选图
              const filteredUnapproved = filteredSelections.filter(
                (s) => !s.approved,
              );
              ...
              const allSelected =
                ...
                filteredUnapproved.every((s) =>
                  tempApprovedSelections.has(s.id),
                );
        ...
        <div className="grid gap-2 grid-cols-1 md:grid-cols-2">
              ...
              {availableBeatmaps.length > 1 && (
                <div className="mb-4">
                  <Dropdown
                    ...
                    onChange={(value) => {
                      const selected = availableBeatmaps.find(
                ...
                <div className="mb-4 p-4 bg-white border border-gray-300 rounded-lg shadow-sm max-w-[660px]">
                  ...
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p
                        className="font-bold text-xs text-gray-600 truncate"
                        title={
                      ...
                      <p className="font-bold text-xs text-gray-600">
                        [{beatmapPreview.version}] by {beatmapPreview.creator}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mb-3 text-xs text-gray-600">
                    <div className="grid grid-cols-4 gap-1">
                      ...
                      <div
                        className={`text-center font-bold text-lg ${selectedMods !== "NM" && moddedStats?.cs !== undefined ? (moddedStats.cs > beatmapPreview.cs + 0.01 ? "text-red-500" : moddedStats.cs < beatmapPreview.cs - 0.01 ? "text-green-500" : "") : ""}`}
                      >
                        {(() => {
                          const val = moddedStats?.cs ?? beatmapPreview.cs;
                          if (
                            selectedMods !== "NM" &&
                            moddedStats?.cs !== undefined
                          ) {
                            if (moddedStats.cs > beatmapPreview.cs + 0.01)
                      ...
                      <div
                        className={`text-center font-bold text-lg ${selectedMods !== "NM" && moddedStats?.ar !== undefined ? (moddedStats.ar > beatmapPreview.ar + 0.01 ? "text-red-500" : moddedStats.ar < beatmapPreview.ar - 0.01 ? "text-green-500" : "") : ""}`}
                      >
                        {(() => {
                          const val = moddedStats?.ar ?? beatmapPreview.ar;
                          if (
                            selectedMods !== "NM" &&
                            moddedStats?.ar !== undefined
                          ) {
                            if (moddedStats.ar > beatmapPreview.ar + 0.01)
                              return `${val.toFixed(2)} ▲`;
                            if (moddedStats.ar < beatmapPreview.ar - 0.01)
                              return `${val.toFixed(2)} ▼`;
                          }
                      ...
                      <div
                        className={`text-center font-bold text-lg ${selectedMods !== "NM" && moddedStats?.od !== undefined ? (moddedStats.od > beatmapPreview.od + 0.01 ? "text-red-500" : moddedStats.od < beatmapPreview.od - 0.01 ? "text-green-500" : "") : ""}`}
                      >
                        {(() => {
                          const val = moddedStats?.od ?? beatmapPreview.od;
                          if (
                            selectedMods !== "NM" &&
                            moddedStats?.od !== undefined
                          ) {
                            if (moddedStats.od > beatmapPreview.od + 0.01)
                              return `${val.toFixed(2)} ▲`;
                            if (moddedStats.od < beatmapPreview.od - 0.01)
                              return `${val.toFixed(2)} ▼`;
                          }
                      ...
                      <div
                        className={`text-center font-bold text-lg ${selectedMods !== "NM" && moddedStats?.hp !== undefined ? (moddedStats.hp > beatmapPreview.hp + 0.01 ? "text-red-500" : moddedStats.hp < beatmapPreview.hp - 0.01 ? "text-green-500" : "") : ""}`}
                      >
                        {(() => {
                          const val = moddedStats?.hp ?? beatmapPreview.hp;
                          if (
                            selectedMods !== "NM" &&
                            moddedStats?.hp !== undefined
                          ) {
                            if (moddedStats.hp > beatmapPreview.hp + 0.01)
                              return `${val.toFixed(2)} ▲`;
                            if (moddedStats.hp < beatmapPreview.hp - 0.01)
                              return `${val.toFixed(2)} ▼`;
                          }
                      ...
                      <div
                        className={`text-center font-bold text-base ${selectedMods === "DT" && customDTRate !== "" ? "text-red-500" : ""}`}
                      >
                        {selectedMods === "DT" && customDTRate !== ""
                          ? formatLength(
                              Math.round(
                                beatmapPreview.total_length /
                                  (customDTRate as number),
                              ),
                            ) + " ▼"
                      ...
                      <div
                        className={`text-center font-bold text-base ${selectedMods !== "NM" && moddedStats?.bpm !== undefined ? (moddedStats.bpm > beatmapPreview.bpm + 0.01 ? "text-red-500" : moddedStats.bpm < beatmapPreview.bpm - 0.01 ? "text-green-500" : "") : ""}`}
                      >
                        {(() => {
                          const val = moddedStats?.bpm ?? beatmapPreview.bpm;
                          if (
                            selectedMods !== "NM" &&
                            moddedStats?.bpm !== undefined
                          ) {
                            if (moddedStats.bpm > beatmapPreview.bpm + 0.01)
                              return `${val} ▲`;
                            if (moddedStats.bpm < beatmapPreview.bpm - 0.01)
                              return `${val} ▼`;
                          }
                      ...
                      <div
                        className={`text-center font-bold text-base ${selectedMods !== "NM" && moddedStats?.starRating !== undefined ? (moddedStats.starRating > beatmapPreview.star_rating + 0.01 ? "text-red-500" : moddedStats.starRating < beatmapPreview.star_rating - 0.01 ? "text-green-500" : "") : ""}`}
                      >
                        {(() => {
                          const val =
                            moddedStats?.starRating ??
                            beatmapPreview.star_rating;
                          if (
                            selectedMods !== "NM" &&
                            moddedStats?.starRating !== undefined
                          ) {
                            if (
                              moddedStats.starRating >
                              beatmapPreview.star_rating + 0.01
                            )
                              return `${val.toFixed(2)} ▲`;
                            if (
                              moddedStats.starRating <
                ...
                <div className="flex flex-cow gap-2 mb-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={modPosition}
                      onChange={(e) =>
                        setModPosition(parseInt(e.target.value) || 1)
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
              ...
              {duplicateWarning.show && (
                <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                  ...
                  <p className="text-yellow-700 text-sm mb-2">
                    此谱面已被选择 {duplicateWarning.existingSelections.length}{" "}
                    次：
                  </p>
                  <ul className="text-sm text-yellow-700 list-disc list-inside">
                    {duplicateWarning.existingSelections.map(
                      (sel: ExistingSelection, index: number) => (
                        <li key={index}>
                          {sel.selectedMods}
                          {sel.modPosition} - {sel.category} -{" "}
                          {sel.selectedByUsername}
                        </li>
                      ),
                    )}
                  </ul>
          ...
          <div className="mb-6 p-4 border border-gray-300 rounded-lg bg-gray-50 object-center">
            ...
            <div>
              ...
              {/* osz文件上传区域 */}
              <p>
                2.上传.osz文件自动解析并填充表单信息，文件将存储为：{season}_
                {category}_{selectedMods}
                {modPosition}_[生成的负数bid].osz
              </p>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="text-gray-500">
                          {(oszFile.size / 1024 / 1024).toFixed(2)} MB
                        </span>
                      </div>
                ...
                <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
                        ...
                        <div className="flex justify-between items-center">
                          <div>
                            ...
                            <div className="text-xs text-gray-600">
                              CS: {beatmapInfo.cs.toFixed(1)} | AR:{" "}
                              {beatmapInfo.ar.toFixed(1)} | OD:{" "}
                              {beatmapInfo.od.toFixed(1)} | HP:{" "}
                              {beatmapInfo.hp.toFixed(1)}
                            </div>
                          </div>
                  ...
                  <div className="mt-3 flex justify-between items-center">
                    <span className="text-sm text-gray-600">
                      已选择:{" "}
                      {oszBeatmapInfos[selectedDifficultyIndex]?.version}
                    </span>
                    <button
                      onClick={() =>
                        handleDifficultySelect(selectedDifficultyIndex)
                      }
                ...
                <div className="flex gap-4">
                  <label className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setCustomPoolType(
                          e.target.value as "original" | "custom",
                        )
                      }
      ...
      <div className="mb-6 relative p-2 w-max bg-[#2d2d2d] rounded-lg hover:scale-[1.01] transition-all">
        <div className="flex relative">
          {/* 滑块 */}
          <div
            className={`absolute h-8 bg-[#E93B66] rounded-lg transition-all duration-300 ease-out ${
              activeTab === "cards" ? "left-1" : "left-1 translate-x-full"
            }`}
            style={{
        ...
        <div className="w-max flex flex-cow bg-[#1a1a1a] rounded-lg">
          <button
            ...
            className={`hover:scale-[1.05] transition-all relative px-4 py-1 rounded-lg gap-1 duration-300 flex items-center z-10 ${
              activeTab === "cards"
                ? "text-white"
          ...
          {Object.entries(getSelectionsByMod()).map(([mod, modSelections]) => (
            <div key={mod} className="space-y-4">
              ...
              <div className="flex items-end gap-3 ">
                <div
                  className={`hover:scale-x-[1.05] transition-all h-8 px-2 mt-4 py-1 rounded-lg font-bold text-lg ${getModColorTextClass(mod)}`}
                >
              ...
              <div className="grid grid-flow-row-dense row-auto grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full">
                {modSelections.map((selection) => (
                  <div
                    key={selection.id}
                    className={`border object-center z-0 relative h-full overflow-hidden rounded-lg p-4 shadow-sm hover:shadow-md hover:scale-x-[1.01] transition-all duration-200 cursor-pointer ${
                      selection.approved
                        ? "border-green-300 border-l-10 bg-white"
                  ...
                  >
                    ...
                    <div className="relative">
                      ...
                      {selection.approved && (
                        <div className="absolute top-65 -left-77 w-full -z-1">
                          <p className="font-bold text-4xl text-green-300 rotate-90">
                            过审APPROVED
                          </p>
                        </div>
                      )}
                      ...
                      <div className="flex flex-col flex-col-1 w-full">
                        <div className="flex items-center gap-2 mb-1">
                          <div className="flex items-center gap-2 flex-1">
                            <span
                              className={`absolute -bottom-3 -right-3 -z-1 hover:scale-[1.05] transition-all px-2 py-1 rounded text-6xl font-bold ${getModColorTextClass(selection.selectedMods)}`}
                            >
                              {selection.selectedMods === "LZ"
                                ? selection.customModName &&
                                  selection.customModName.trim() !== ""
                                  ? `LZ${selection.modPosition}-${selection.customModName}`
                                  : `LZ${selection.modPosition}`
                                : selection.selectedMods === "DT"
                                  ? selection.customDTRate &&
                                    selection.customDTRate !== 1.5
                            ...
                            {category === "all" && (
                              <span className="px-2 py-1 bg-gray-600 text-white text-xs rounded hover:scale-[1.05] transition-all">
                                {CATEGORY_OPTIONS.find(
                                  (cat) => cat.value === selection.category,
                          ...
                          <div className="flex items-center gap-2 justify-end">
                            ...
                            {permissions.isadmin && !selection.approved && (
                              <label className="flex items-center">
                                <input
                                  type="checkbox"
                                  checked={tempApprovedSelections.has(
                                    selection.id,
                                  )}
                                  onChange={(e) => {
                                    const newSelections = new Set(
                                      tempApprovedSelections,
                                    );
                                    if (e.target.checked) {
                                      newSelections.add(selection.id);
                                    } else {
                    ...
                    <p className="font-bold text-xs text-gray-200">
                      [{selection.version}] by {selection.creator}
                    </p>
                    <div className="flex flex-row gap-0 content-end items-end">
                      <h3
                        className="font-bold text-white text-2xl truncate"
                        title={selection.title_unicode || selection.title}
                      >
                        {selection.title_unicode || selection.title}
                      </h3>
                      <p
                        className="font-bold text-xs relative bottom-1 text-gray-200 truncate"
                        title={selection.artist_unicode || selection.artist}
                      >
                    ...
                    <div className="mb-3 text-xs text-gray-600">
                      <div className="grid grid-cols-1 gap-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
                        <div
                          className={`hover:scale-[1.05] transition-all bg-white rounded-lg text-center font-medium flex flex-cow gap-1 items-start p-1 ${getModColorBorderClass(selection.selectedMods)}`}
                        >
                          <Diameter size={16} />
                          CS
                        ...
                        <div className="text-center font-medium flex flex-cow gap-1 items-center items-center p-1">
                          <Hourglass size={16} />
                          <div className="text-center text-base">
                            {formatLength(selection.totalLength)}
                          </div>
                        </div>
                        <div className="text-center font-medium flex flex-cow gap-1 items-center items-center p-1">
                          <CircleStar size={16} />
                        ...
                        <div
                          ...
                          className={`hover:scale-[1.05] transition-all bg-white rounded-lg text-center font-medium flex flex-cow gap-1 items-start p-1`}
                        >
                          <Star size={16} />
                          Star
                      ...
                      <div className="flex items-center gap-3 w-full">
                        <span className="flex items-center gap-1">
                          <Image
                            ...
                            alt={selection.selectedByUsername || "未知"}
                            width={16}
                            height={16}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-6 max-w-4xl w-full max-h-[80vh] overflow-y-auto">
              ...
              <p className="text-gray-600 mb-4">
                以下 {tempApprovedSelections.size}{" "}
                个选图将被过审并公开，请确认：
              </p>
                ...
                <table className="table-auto w-full text-sm border-collapse border border-gray-300">
                  ...
                  <tbody>
                    {selections
                      .filter(
                        (selection) =>
                          tempApprovedSelections.has(selection.id) &&
                          !selection.approved,
                      )
                      .map((selection) => (
                        <tr key={selection.id} className="hover:bg-gray-50">
                          <td className="border border-gray-300 px-3 py-2 text-left">
                            <span
                              className={`${getModColorClass(selection.selectedMods)} text-white px-2 py-1 rounded text-sm font-bold`}
                            >
                              {selection.selectedMods === "LZ"
                                ? selection.customModName &&
                                  selection.customModName.trim() !== ""
              ...
              <div className="flex gap-4 justify-end mt-6">
                ...
                >
                  {isBulkApproving
                    ? "过审中..."
                    : `确认过审 (${tempApprovedSelections.size} 个)`}
                </button>
              </div>
            </div>
          </div>
              ...
              if (isCustomMap) {
                ...
                firstColumn.push(
                  <button
                    ...
                    onClick={() => {
                      ...
                      const category =
                        contextMenu.selection!.category || "qualification";
                      const selectedMods =
                        contextMenu.selection!.selectedMods || "NM";
                      const modPosition =
                        contextMenu.selection!.modPosition || 1;
                ...
                firstColumn.push(
                  ...
                  <button
                    ...
                    onClick={() => {
                      window.open(
                        `osu://b/${contextMenu.selection!.beatmapId}`,
                        "_blank",
                      );
                  ...
                  >
                    <Image
                      src="/icons/osu-lazer-logo-black.svg"
                      alt="viewOsu"
                      width={24}
                      height={24}
                    />
                  ...
                  <button
                    ...
                    onClick={() => {
                      togglePadding(
                        ...
                        contextMenu.selection!.padding || false,
                      );
                      closeContextMenu();
                    }}
                    className="w-full text-left px-4 py-2 text-sm hover:border-b-4 border-gray-400 hover:bg-gray-200 hover:scale-[1.01] transition-all flex items-center gap-2 rounded-lg"
                ...
                (permissions.isadmin) && (
                  <button
                    ...
                    onClick={() => {
                      toggleApproval(
                        contextMenu.selection!.id,
                        contextMenu.selection!.approved,
                      );
                      closeContextMenu();
                    }}
                    className="w-full text-left px-4 py-2 text-sm hover:border-b-4 border-gray-400 hover:bg-gray-200 hover:scale-[1.01] transition-all flex items-center gap-2 rounded-lg"
                ...
                <button
                  onClick={() => {
                    ...
                    const updates: {
                      ...
                      ar: parseFloat(
                        (document.getElementById("edit-ar") as HTMLInputElement)
                          ?.value || "0",
                      ),
                      od: parseFloat(
                        (document.getElementById("edit-od") as HTMLInputElement)
                          ?.value || "0",
                      ),
                      cs: parseFloat(
                        (document.getElementById("edit-cs") as HTMLInputElement)
                          ?.value || "0",
                      ),
                      hp: parseFloat(
                        (document.getElementById("edit-hp") as HTMLInputElement)
                          ?.value || "0",
                      ),
                      bpm: parseFloat(
                        (
                    ...
                    if (editDialog.selection.selectedMods === "LZ") {
                      const customModNameInput = document.getElementById(
                        "edit-customModName",
                      ) as HTMLInputElement;
                      if (customModNameInput) {
                        updates.customModName =
                          customModNameInput.value || undefined;
                      }
                    }

                    // 添加 DT 自定义倍率字段
                    if (editDialog.selection.selectedMods === "DT") {
                      const customDTRateInput = document.getElementById(
                        "edit-customDTRate",
                      ) as HTMLInputElement;
                      if (customDTRateInput && customDTRateInput.value) {
                        updates.customDTRate = parseFloat(
                          customDTRateInput.value,
                        );
                      }
                    }
