        ...
        const fetchStats = async () => {
            try {
                ...
                for (const season of seasons) {
                    try {
                        const mappoolResponse = await fetch(`/api/mappool?season=${season}`);
                ...
                if (schedulesResponse.ok) {
                    ...
                    const replayCount = schedulesData.schedules?.filter((schedule: any) => schedule.replay_link)?.length || 0;
