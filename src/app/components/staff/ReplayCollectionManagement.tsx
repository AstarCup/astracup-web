...
export default function ReplayCollectionManagement({
  user,
  permissions,
}: ReplayCollectionManagementProps) {
  ...
  const [selectedCategory, setSelectedCategory] = useState(() => {
    if (typeof window !== "undefined") {
      return (
        localStorage.getItem("replayCollection_category") || "qualification"
      );
    }
  ...
  const loadUploadedUsers = async () => {
    ...
    try {
      const response = await fetch(
        `/api/uploaded-users?season=${selectedSeason}&category=${selectedCategory}`,
      );
      if (response.ok) {
        ...
        console.error(
          "Failed to load uploaded users, response not ok:",
          response.status,
        );
      }
    } catch (error) {
  ...
  const getFilteredMaps = () => {
    ...
    if (selectedModFilter === "all") {
      ...
      filteredMaps = paddingMaps.filter(
        (map) => map.selectedMods === selectedModFilter,
      );
    }
      ...
      try {
        ...
        if (response.ok) {
          ...
          const formattedData = (data.selections || []).map((map: any) => ({
            ...
            // 提名者信息 - 用于详细信息卡片显示
            selectedByUsername:
              map.selectedByUsername || map.selectedBy || "未知",
  ...
  const handleDownloadAllReplays = async () => {
    ...
    try {
      const response = await fetch(
        `/api/download-all-replays?season=${selectedSeason}&category=${selectedCategory}`,
      );
                ...
                <div key={mod} className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-4">
                    {maps.map((map) => (
                      <div
                        key={map.id}
                        className={`border rounded-md p-4 shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer relative overflow-hidden w-100 h-64 ${highlightedMapId === map.id ? "ring-4 ring-blue-500 ring-opacity-75 shadow-lg highlight-pulse" : ""} ${
                          !isLoadingUploadedUsers &&
                          uploadedUsers[
                            `${selectedSeason}/${selectedCategory}/${map.BID}`
                          ]?.length > 0
                            ? "border-green-500"
                            : "border-gray-300"
                        }`}
                      ...
                      >
                        ...
                        <div
                          className={`absolute inset-0 ${!isLoadingUploadedUsers && uploadedUsers[`${selectedSeason}/${selectedCategory}/${map.BID}`]?.length > 0 ? "bg-gradient-to-tr from-green-100 via-green-50/80 to-transparent" : "bg-gradient-to-tr from-white via-white/80 to-transparent"}`}
                        ></div>
                        ...
                        <div className="relative z-10 h-full flex flex-col">
                          <div className="mb-3 flex items-start justify-between">
                            ...
                            <div className="flex-shrink-0">
                              <span
                                className={`font-bold text-lg ${getModColor(map.selectedMods)} px-2 py-1 shadow-sm bg-white/90 rounded`}
                              >
                                {map.selectedMods}
                                {map.modPosition}
                              </span>
                            ...
                            <div className="flex-1 min-w-0 ml-3 text-right">
                              <h4
                                className="font-bold text-sm text-gray-800 truncate px-2 py-1"
                                title={`${map.artist} - ${map.title}`}
                              >
                                {map.artist} - {map.title}
                              </h4>
                              <p
                                className="text-xs text-gray-600 truncate px-2 py-1 mt-1"
                                title={map.version}
                              >
                                [{map.version}]
                              </p>
                          ...
                          <div className="mb-3 bg-white rounded-md px-2 flex-grow h-[24px]">
                            ...
                            {!isLoadingUploadedUsers &&
                              ...
                              uploadedUsers[
                                ...
                                <div className="text-xl text-bold text-green-600 font-medium mt-1 px-2 py-1">
                                  ✓ 你已上传
                                </div>
                              )}
                          </div>
                          ...
                          {uploading === map.id && (
                            <div className="absolute inset-0 flex items-center justify-center z-30">
                              <div className="text-sm text-blue-600 bg-white px-3 py-2 rounded shadow-lg">
                                上传中...
                              </div>
                            </div>
                          )}
