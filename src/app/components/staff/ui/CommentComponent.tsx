...
import { MessageCirclePlus, MessageCircleDashed } from "lucide-react";
  ...
  const handleDeleteComment = async (id: number) => {
    ...
    try {
      const response = await fetch(
        `/api/map-ratings?id=${id}&userId=${userId}`,
        { method: "DELETE" },
      );
      if (response.ok) {
        showSuccess("评论已删除");
  ...
  return (
    <div className="space-y-3">
      {compactMode ? (
        // 紧凑模式：横向显示评论
        <div>
          <div className="flex items-end justify-end mb-2"></div>
          {comments.length === 0 ? (
            <div className="">
              <button
            ...
            <div className="flex flex-wrap gap-2">
              {comments.map((c) => (
                <div
                  key={c.id}
                  className="hover:scale-[1.05] transition-all rounded-full relative group inline-flex items-center gap-2 pr-4 bg-[#E93B66] max-w-xs hover:bg-[#E93B66]/90"
                >
                  {/* 头像 */}
                  <div className="w-10 h-10 rounded-full overflow-hidden border border-gray-300 flex-shrink-0 relative">
                    ...
                    />
                    <div
                      className="avatar-fallback w-full h-full bg-gray-300 rounded-full flex items-center justify-center absolute inset-0"
                      style={{ display: "none" }}
                    >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {userId === c.userId && (
                      <button
                        ...
                        title={
                          deletingCommentId === c.id ? "删除中..." : "删除评论"
                        }
