...
function RollingDigit({ value }: { value: number }) {
  ...
  return (
    <div className="relative w-[120px] h-[80px] overflow-hidden inline-block mx-1">
      ...
      >
        {digits.map((d, i) => (
          <div key={i} className="text-[70px] leading-[80px] text-center">
            {d}
          </div>
        ))}
      </div>
...
const Countdown = () => {
  ...
  const [timeLeft, setTimeLeft] = useState(() =>
    calculateTimeLeft("2100-01-01T00:00:00"),
  );
    ...
    return (
      <div className={`countdown ${audiowide.className} antialiased font-bold`}>
        <div className="flex flex-wrap flex-row justify-end">
          <RollingDigit value={0} />
          <div className="pt-7">天</div>
          <RollingDigit value={0} />
          <div className="pt-7">时</div>
          <RollingDigit value={0} />
          <div className="pt-7">分</div>
          <RollingDigit value={0} />
          <div className="pt-7">秒</div>
        </div>
      </div>
    );
  if (timeLeft.isExpired)
    return <div className="text-[80px] font-bold">活动已开始！</div>;

  return (
    <div className={`countdown ${audiowide.className} antialiased font-bold`}>
      <div className="flex flex-wrap flex-row justify-end">
        <RollingDigit value={timeLeft.days} />
        <div className="pt-7">天</div>
        <RollingDigit value={timeLeft.hours} />
        <div className="pt-7">时</div>
        <RollingDigit value={timeLeft.minutes} />
        <div className="pt-7">分</div>
        <RollingDigit value={timeLeft.seconds} />
        <div className="pt-7">秒</div>
      </div>
    </div>
  );
