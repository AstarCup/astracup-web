  ...
  const startBulkDownload = async (source: "nerinyan" | "sayobot" | "osu") => {
    ...
    try {
      ...
      for (let i = 0; i < bulkDownloadItems.length; i++) {
        ...
        try {
          // 更新当前项目状态为下载中
          setBulkDownloadItems((prev) =>
            prev.map((prevItem, idx) =>
              idx === i
                ? { ...prevItem, status: "downloading", progress: 10 }
                : prevItem,
            ),
          );
          ...
          console.log(
            `Downloading beatmap ${i + 1}/${bulkDownloadItems.length}:`,
            item.sid,
            item.bid,
          );
          ...
          const response = await fetch(
            `/api/download-beatmap?sid=${item.sid}&source=${source}`,
            {
              method: "GET",
              headers: {
      ...
      if (successCount > 0) {
        ...
        try {
          ...
          showError(
            `ZIP文件生成失败: ${zipError instanceof Error ? zipError.message : "未知错误"}`,
          );
          return;
        }
      } else {
      ...
      items.push(
        ...
        // 下载相关
        {
          ...
          onClick: () => {
            ...
            const bidStr =
              beatmapId < 0 ? `-${Math.abs(beatmapId)}` : beatmapId.toString();
  ...
  return (
    <div className="mb-20">
      <div className="flex justify-between items-start mb-0">
        ...
        <div className="flex space-x-3">
          ...
          <button
            ...
            title={
              showOriginalTitle ? "切换为显示罗马化标题" : "切换为显示原曲名"
            }
          >
          ...
          <tbody>
            {data.map((row, idx) => {
              ...
              const hasUploads =
                season &&
                category &&
                uploadedUsers[`${season}/${category}/${row.BID}`]?.length > 0;
              ...
              if (row.customModName && row.Slot?.startsWith("LZ")) {
                ...
                row.customDTRate &&
                row.customDTRate !== 1.5 &&
                row.Slot?.startsWith("DT")
              ) {
              ...
              if (row.Slot?.includes("NM")) {
                ...
                slotClass =
                  "bg-yellow-300 p-2 text-black text-center font-bold";
                ...
                slotClass =
                  "bg-purple-400 p-2 text-white text-center font-bold";
      ...
      {detailCard?.visible && detailCard.row && (
        <div
          className="fixed shadow-lg w-[500px] transition-all duration-200 z-999"
          style={{
        ...
        >
          <div
            className={`border p-4 shadow-sm hover:shadow-md transition-all duration-200 border-gray-300 bg-white`}
          >
            {/* 头部：封面和基本信息 */}
            <div className="flex items-start gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <h3
                  ...
                  title={detailCard.row.title || detailCard.row.MapInfo}
                >
                ...
                <p className="font-bold text-xs text-gray-600">
                  by {detailCard.row.creator || detailCard.row._Creator}
                </p>
              </div>
            </div>
            ...
            <div className="mb-3 text-xs text-gray-600">
              <div className="grid grid-cols-4 gap-1">
                ...
                <div className="text-center font-bold text-lg">
                  {Number(detailCard.row.cs || detailCard.row.CS || 0).toFixed(
                    1,
                  )}
                </div>
                <div className="text-center font-bold text-lg">
                  {Number(detailCard.row.ar || detailCard.row.AR || 0).toFixed(
                    1,
                  )}
                </div>
                <div className="text-center font-bold text-lg">
                  {Number(detailCard.row.od || detailCard.row.OD || 0).toFixed(
                    1,
                  )}
                </div>
                <div className="text-center font-bold text-lg">
                  {Number(detailCard.row.hp || 0).toFixed(1)}
                </div>
                ...
                <div className="text-center font-bold text-base">
                  {formatLength(
                    detailCard.row.totalLength || detailCard.row.HitLength || 0,
                  )}
                </div>
                <div className="text-center font-bold text-base">
                  {Number(
                    detailCard.row.maxCombo || detailCard.row.MaxCombo || 0,
                  ).toFixed(0)}
                </div>
                <div className="text-center font-bold text-base">
                  {Math.round(detailCard.row.bpm || detailCard.row.BPM || 0)}
                </div>
                <div className="text-center font-bold text-base">
                  {Number(
                    detailCard.row.starRating || detailCard.row.SR || 0,
                  ).toFixed(2)}
                </div>
              </div>
            </div>
            ...
            <div className="mb-3 text-xs text-gray-600">
              <div className="flex items-center gap-3 w-full">
                <span className="flex items-center gap-1">
                  提名者: {detailCard.row.selectedByUsername || "未知"}
                </span>
              </div>
            </div>
          </div>
