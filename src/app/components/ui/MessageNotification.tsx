                ...
                {messages.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {messages.map((message) => (
                      <div
                        key={message.id}
                        className="bg-gray-100 border-b-4 group rounded-lg border-yellow-400 hover:border-gray-600 hover:bg-gray-200 active:scale-[0.99] hover:scale-[1.01] transition-all duration-200 p-3"
                      >
                        ...
                        <p className="text-gray-300 group-hover:text-gray-600 text-left text-sm mb-2">
                          {message.content}
                        </p>
                        ...
                        {/* 响应按钮 */}
                        {message.type === "match_invitation" &&
                          message.status === "unread" && (
                            <div className="flex flex-col sm:flex-row gap-2 justify-end">
                              <button
                                onClick={() =>
                                  updateMessageStatus(message.id, "accept")
                                }
                                disabled={loading}
                              ...
                              <button
                                onClick={() =>
                                  updateMessageStatus(message.id, "decline")
                                }
                                disabled={loading}
