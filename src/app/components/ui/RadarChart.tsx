...
export default function RadarChart({
  ...
  useEffect(() => {
    ...
    const baseSkills = [
      { label: "准度", key: "accuracy" },
      { label: "体力", key: "stamina" },
      { label: "初见力", key: "firstSight" },
    ...
    if (customData && customData.key && customData.value !== undefined) {
      skills.push({ label: customData.key, key: "custom" });
    }
    ...
    for (let level = 1; level <= 6; level++) {
      ...
      // 在最后一个多边形外添加标签
      if (level === 6) {
        const labelValue = level * 2; // 每个网格代表2分
        const labelAngle = Math.PI / 2; // 在顶部显示
        const labelX = centerX + Math.cos(labelAngle) * (levelRadius + 15);
        const labelY = centerY + Math.sin(labelAngle) * (levelRadius + 15);
    ...
    const points = skills.map((skill, i) => {
      let value = 0;
      if (skill.key === "custom" && customData) {
        value = customData.value || 0;
      } else {
        value = data[skill.key as keyof typeof data] || 0;
      }
      // 计算最大值用于归一化
