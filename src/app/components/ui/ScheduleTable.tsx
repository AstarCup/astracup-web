    ...
    return (
      <div className={`absolute ${positionClasses} z-10`}>
        <div className="relative">
          ...
          {/* 小三角形箭头 */}
          <div
            className={`absolute w-2 h-2 bg-black transform rotate-45 -z-2 ${arrowClasses}`}
          ></div>
        </div>
      </div>
    );
          ...
          {rounds.map((round) => (
            <React.Fragment key={round}>
              ...
              <tr className="bg-gray-100 font-bold">
                <td
                  className="px-6 py-4 text-lg font-bold text-gray-800"
                  colSpan={10}
                >
                  <div className="flex items-center">
                    ...
                    <button
                      onClick={() =>
                        (window.location.href = `/multiplayer-scores?round_number=${groupedSchedule[round][0].round_number}`)
                      }
              ...
              {groupedSchedule[round].map((item, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <span
                      className={`inline-flex px-2 py-1 text-xs font-semibold ${getStatusColor(item.status)}`}
                    >
                      {item.status}
                    </span>
                  </td>
