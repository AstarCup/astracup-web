...
export default function UserProfile({ user, onLogout }: UserProfileProps) {
  ...
  if (!user) {
    return (
      <div className="">{/* <p className="text-gray-600">未登录</p> */}</div>
    );
  }

  const formatRank = (rank: number | null) => {
    if (rank === null || rank === undefined) return "未排名";
      ...
      <div className="relative">
        <div className="grid grid-row-1 bg-white rounded-lg mt-3 gap-10 text-sm items-start justify-start text-right w-full p-6">
          ...
          <div className="items-end justify-between relative">
            <span className="text-gray-400 font-bold text-2xl absolute top-0 left-0 text-left mb-1">
              PP
            </span>
            <span
              className={`text-6xl text-gray-600 font-bold relative text-right top-4 text-pink-500`}
            >
              {Math.round(user.pp || 0)}
            </span>
          </div>
          <div className="items-end justify-between relative">
            <span className="text-gray-400 font-bold text-2xl absolute top-0 left-0 text-left mb-1">
            ...
            <span
              className={`text-4xl text-gray-600 font-bold relative text-right top-4 text-pink-400`}
            >
          ...
          {user.country_rank && (
            <div className="items-end justify-between relative">
              <span className="text-gray-400 font-bold text-2xl absolute top-0 left-0 text-left mb-1">
                地区排名-<a className="text-xl">{user.country}</a>
              </span>
              <span
                className={`text-4xl text-gray-600 font-bold relative text-right top-4 text-pink-400`}
              >
