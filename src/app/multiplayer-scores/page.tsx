...
export default function MultiplayerScoresPage() {
  ...
  const [selectedRoom, setSelectedRoom] = useState<MultiplayerRoom | null>(
    null,
  );
  ...
  const [approvedPlayers, setApprovedPlayers] = useState<Set<string>>(
    new Set(),
  );
  ...
  const loadMapSelectionsByCategory = async (category: string) => {
    ...
    try {
      const response = await fetch(
        `/api/map-selections?approved=true&category=${category}`,
      );
  ...
  const loadScoresByRoundNumber = async (roundNumber: string) => {
    ...
    try {
      ...
      if (matchSchedulesData.success) {
        // 过滤出该轮次的比赛
        const roundSchedules = matchSchedulesData.schedules.filter(
          (schedule: any) => {
        ...
        const roomIds = roundSchedules
          .map((schedule: any) => {
            ...
            if (!roomId) {
              console.error(
                `Failed to extract roomId from match_link: ${schedule.match_link}`,
              );
            }
            return roomId;
          })
        ...
        for (const roomId of uniqueRoomIds) {
          try {
            ...
            const scoresResponse = await fetch(
              `/api/match-scores/save?roomId=${roomId}`,
            );
            ...
            if (
              ...
              // 为每个分数添加房间信息
              const scoresWithRoomInfo = scoresData.scores.map(
                (score: DisplayScore) => ({
                  ...score,
                  roomId: roomId,
              ...
              roomsWithData.push(roomId as string);
              // console.log(`Added ${scoresWithRoomInfo.length} scores from database room ${roomId}`);
            } else {
              // 该房间没有数据，需要从API获取
              roomsWithoutData.push(roomId as string);
              // console.log(`Room ${roomId} has no data in database, will fetch from API`);
            }
          } catch (err) {
            console.error(
              `Error loading scores from database for room ${roomId}:`,
              err,
            );
            roomsWithoutData.push(roomId as string);
          }
        ...
        // console.log(`Rooms with data: ${roomsWithData.length}, Rooms without data: ${roomsWithoutData.length}`);

        // 如果有房间没有数据，从osu API获取这些房间的分数
        if (roomsWithoutData.length > 0) {
          // console.log(`从osu API获取 ${roomsWithoutData.length} 个房间的分数...`);
          ...
          if (data.success) {
            ...
            // 将新获取的分数与数据库分数合并
            allDatabaseScores.push(...data.scores);
            ...
            try {
              ...
              if (sessionData.success && userSession?.osuId) {
                currentUserOsuId = userSession.osuId;
              }
            } catch (err) {
            ...
            if (currentUserOsuId && data.scores.length > 0) {
              try {
                ...
                for (const [roomId, scores] of Object.entries(scoresByRoom)) {
                  ...
                  if (roomInfo) {
                    ...
                    if (saveData.success) {
                      ...
                      console.error(
                        `[Round Save] Failed to save scores for room ${roomId}:`,
                        saveData.error,
                      );
                    }
                  }
                ...
                setError(
                  `从数据库加载了 ${roomsWithData.length} 个房间的数据，从osu API获取并保存了 ${data.scores.length} 条新分数记录，涵盖 ${Object.keys(scoresByRoom).length} 个房间。`,
                );
              } catch (err) {
                console.error(
                  "[Round Save] Error saving scores to database:",
                  err,
                );
                setError(
                  `从数据库加载了 ${roomsWithData.length} 个房间的数据，从osu API获取了 ${data.scores.length} 条分数记录，但保存到数据库时出错。`,
                );
              }
            } else {
              setError(
                `从数据库加载了 ${roomsWithData.length} 个房间的数据，从osu API获取了 ${data.scores.length} 条分数记录。`,
              );
            }
          } else {
            console.error("Batch API failed:", data.error);
            setError(
              `从osu API获取分数失败: ${data.error}。已从数据库加载了 ${roomsWithData.length} 个房间的数据。`,
            );
          }
        } else {
          // 所有房间都有数据
          setError(
            `成功从数据库加载了 ${allDatabaseScores.length} 条分数记录，涵盖 ${roomsWithData.length} 个房间。`,
          );
        }
  ...
  const loadMapSelections = async () => {
    ...
    try {
      const response = await fetch(
        "/api/map-selections?approved=true&category=qualification",
      );
      const data = await response.json();
    ...
    try {
      ...
      if (data.success && data.rooms && data.rooms.length > 0) {
        ...
        for (const room of data.rooms) {
          try {
            const scoresResponse = await fetch(
              `/api/match-scores/save?roomId=${room.id}`,
            );
            const scoresData = await scoresResponse.json();
  ...
  const filterScores = (scores: DisplayScore[]): DisplayScore[] => {
    ...
    const filtered = scores.filter((score) =>
      approvedPlayers.has(score.user_id.toString()),
    );
  ...
  const loadAllScoresWithRoom = async (
      ...
      const allScoresPromises = room.playlist.map(async (playlistItem) => {
        ...
        try {
          ...
          if (data.success) {
            ...
            console.error(
              `Failed to load scores for playlist ${playlistItem.id}:`,
              data.error,
            );
            return [];
          }
          ...
          console.error(
            `Error loading scores for playlist ${playlistItem.id}:`,
            err,
          );
          return [];
        }
      ...
      if (!databaseHasData) {
        ...
        if (data.success && data.rooms.length > 0) {
          ...
          if (allApiScores.length > 0) {
            try {
              ...
              if (currentUserOsuId) {
                console.log(
                  `保存房间 ${roomId} 的 ${allApiScores.length} 条分数到数据库...`,
                );
                ...
                if (saveData.success) {
                  console.log(
                    `成功保存房间 ${roomId} 的 ${saveData.data.scores_count} 条分数到数据库`,
                  );
                  setError(
                    `从osu! API获取并保存了 ${saveData.data.scores_count} 条分数记录`,
                  );
                } else {
                  console.error(
                    `保存房间 ${roomId} 分数到数据库失败:`,
                    saveData.error,
                  );
                  setError(
                    `从osu! API获取了 ${allApiScores.length} 条分数，但保存到数据库时出错: ${saveData.error}`,
                  );
                }
              } else {
                console.warn("用户未登录，无法保存分数到数据库");
                setError(
                  `从osu! API获取了 ${allApiScores.length} 条分数，但用户未登录无法保存到数据库`,
                );
              }
            } catch (err) {
              console.error("保存分数到数据库时出错:", err);
              setError(
                `从osu! API获取了 ${allApiScores.length} 条分数，但保存到数据库时出错`,
              );
            }
          }
        } else {
        ...
        if (roomInfo) {
          room = {
            ...
            queue_mode: (roomInfo.queue_mode || "all_players") as
              | "all_players"
              | "host_only"
  ...
  const getMapSelectionForPlaylistItem = (
    playlistItem: any,
  ): MapSelection | null => {
  ...
  const saveScoresToDatabase = async () => {
    ...
    try {
      ...
      const currentUserOsuId = userSession.osuId;
        ...
        for (const [roomId, roomScores] of Object.entries(scoresByRoom)) {
          try {
            ...
            const roomResponse = await fetch(
              `/api/multiplayer/rooms?roomId=${roomId}`,
            );
            ...
            if (roomData.success && roomData.rooms.length > 0) {
              ...
              if (saveData.success) {
                ...
                console.error(
                  `[Round Save] 保存房间 ${roomId} 失败:`,
                  saveData.error,
                );
                setSaveError(
                  `保存房间 ${room.name} 的分数失败: ${saveData.error}`,
                );
              }
            } else {
            ...
            console.error(
              `[Round Save] 保存房间 ${roomId} 的分数数据时发生错误:`,
              err,
            );
    ...
    <div className="container mx-auto py-8 max-w-full">
        ...
        <div className="bg-[#2D2D2D] p-6 mb-6 border-l-4 border-[#E93B66]">
          ...
          <div className="flex gap-4 mb-4 flex-wrap">
            ...
            <button
              ...
              disabled={
                savingScores || (!selectedRoom && !databaseScores.length)
              }
            ...
            <button
              ...
              disabled={
                updatingScores || !selectedRoom || allScores.length === 0
              }
              className={`px-4 py-2 rounded font-medium transition ${
                updatingScores || !selectedRoom || allScores.length === 0
                  ? "bg-gray-600 text-gray-400 cursor-not-allowed"
          ...
          {savedRooms.length === 0 && (
            <div className="text-gray-400 text-sm">暂无已保存的房间数据</div>
          )}
        </div>
      )}

      {/* 错误提示 */}
      {error && <div className="bg-red-500 text-white p-4 mb-6">{error}</div>}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-white">
            ...
            {selectedRoom.ends_at && (
              <div>
                ...
                <p className="font-bold">
                  {new Date(selectedRoom.ends_at).toLocaleString("zh-CN")}
                </p>
              </div>
            )}
          </div>
      ...
      {!new URLSearchParams(window.location.search).get("round_number") && (
        <div className="flex border-b border-gray-600 mb-6">
          <button
            className={`px-6 py-3 font-medium text-lg transition ${
      ...
      {activeTab === "byPlaylist" &&
        selectedRoom &&
        selectedRoom.playlist.length > 0 && (
          <div className="bg-[#3D3D3D] p-6 mb-6">
            <h2 className="text-xl font-bold text-white mb-4">选择图池</h2>
            <div className="flex gap-4 overflow-x-auto">
              {selectedRoom.playlist.map((playlistItem) => {
                const mapSelection =
                  getMapSelectionForPlaylistItem(playlistItem);
                const hasCover = mapSelection?.coverUrl;

                return (
                  <div
                    key={playlistItem.id}
                    className={`p-4 rounded cursor-pointer transition relative overflow-hidden ${
                      selectedPlaylist === playlistItem.id
                  ...
                  >
                    {/* 半透明遮罩层 */}
                    <div
                      className={`absolute inset-0 ${hasCover ? "bg-black/70" : ""} ${selectedPlaylist === playlistItem.id ? "bg-[#E93B66]/80" : ""}`}
                    ></div>
                        ...
                        {mapSelection && (
                          <div className="flex items-center space-x-2">
                            <div className="flex space-x-1">
                              <span
                                className={`px-2 text-xs font-bold ${getModColorClass(mapSelection.selectedMods)}`}
                              >
                                {mapSelection.selectedMods}
                                {mapSelection.modPosition}
                              </span>
          ...
          {new URLSearchParams(window.location.search).get("round_number") ? (
            ...
            <>
                    ...
                    <MultiplayerScoresTable
                      scores={
                        databaseScores.length > 0
                          ? databaseScores.filter((score) => {
                              const scoreBeatmapId =
                                (score as any).beatmapId ||
                                (score as any).beatmap?.id;
                              const playlistInfo = getSelectedPlaylistInfo();
                              return (
                                playlistInfo &&
                                scoreBeatmapId === playlistInfo.beatmap_id
                              );
                            })
                          : filteredScores
                      }
                      title={getPageTitle()}
                      loading={
                        loadingScores || loadingPlayers || loadingDatabaseScores
                      }
              ...
              {activeTab === "byTotal" && (
                <TotalScoresByModTable
                  scores={
                    databaseScores.length > 0 ? databaseScores : allScores
                  }
                  mapSelections={mapSelections}
                  approvedPlayers={approvedPlayers}
                  currentBeatmapId={undefined} // 按总分不需要当前图池ID
