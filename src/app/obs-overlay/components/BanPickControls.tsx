...
export default function BanPickControls({
  ...
  const seasonOptions = [{ value: "s1", label: "第1赛季" }];
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="bg-red-500/20 p-3 rounded border border-red-500">
          ...
          <div className="text-white">
            Ban: {banPickState.remainingBans.red} | Pick:{" "}
            {banPickState.remainingPicks.red}
          </div>
        </div>
        <div className="bg-blue-500/20 p-3 rounded border border-blue-500">
          <div className="text-white text-sm font-medium">蓝队</div>
          <div className="text-white">
            Ban: {banPickState.remainingBans.blue} | Pick:{" "}
            {banPickState.remainingPicks.blue}
          </div>
        </div>
      </div>
            ...
            banPickState.history
              ...
              .map((record, index) => (
                <div key={index} className="text-white text-sm mb-1">
                  <span
                    className={
                      record.team === "red" ? "text-red-400" : "text-blue-400"
                    }
                  >
                  ...
                  <span className="text-gray-300">#{record.beatmapId}</span>
                </div>
              ))
          )}
        </div>
