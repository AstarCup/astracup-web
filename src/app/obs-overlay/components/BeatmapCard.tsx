...
export default function BeatmapCard({
  ...
  const [animationType, setAnimationType] = useState<"pick" | "ban" | null>(
    null,
  );
  ...
  const getCardClass = () => {
    const baseClass =
      "w-full h-24 flex rounded-lg overflow-hidden cursor-pointer transition-all duration-300 relative";
    ...
    if (beatmap.status === "available") {
      ...
      const teamColor =
        beatmap.bannedBy === "red" ? "border-gray-500" : "border-gray-500";
      return `${baseClass} ${hoverClass} ${flashClass} ${teamColor} border-8`;
    } else if (beatmap.status === "picked") {
      const teamColor =
        beatmap.pickedBy === "red" ? "border-red-500" : "border-blue-500";
      return `${baseClass} ${hoverClass} ${flashClass} ${teamColor} border-8`;
    }
  ...
  const getOverlayClass = () => {
    if (beatmap.status === "banned") {
      return beatmap.bannedBy === "red" ? "bg-red-500/40" : "bg-blue-500/40";
    } else if (beatmap.status === "picked") {
      return beatmap.pickedBy === "red" ? "bg-red-500/40" : "bg-blue-500/40";
    }
    return "";
  };
  ...
  return (
    ...
    >
      ...
      <div className="w-[100px] h-full flex items-center justify-center bg-black/50 ">
        <span
          className={`text-2xl font-bold p-2 w-[80px] text-center rounded ${getModColor()}`}
        >
          {beatmap.slot}
        </span>
      </div>
      ...
      {(beatmap.status === "banned" || beatmap.status === "picked") && (
        <div
          className={`absolute inset-0 flex items-center justify-center ${getOverlayClass()}`}
        >
          <div className="text-center text-white font-bold text-3xl leading-tight flex flex-col items-center gap-2">
            ...
            <div className="">
              {beatmap.status === "banned" ? (
                ...
                <span
                  className={`${beatmap.pickedBy === "red" ? "text-red-500" : "text-blue-500"} bg-white p-2 rounded flex items-center gap-2`}
                >
