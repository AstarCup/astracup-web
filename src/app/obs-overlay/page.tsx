...
export default function ObsOverlay() {
    ...
    try {
      ...
      if (savedSettings) {
        ...
        return {
          ...
          showAvatars:
            parsedSettings.showAvatars !== undefined
              ? parsedSettings.showAvatars
              : true,
          currencyEnabled:
            parsedSettings.currencyEnabled !== undefined
              ? parsedSettings.currencyEnabled
              : false,
          currencyName: parsedSettings.currencyName || "金币",
          redCurrency:
            parsedSettings.redCurrency !== undefined
              ? parsedSettings.redCurrency
              : 0,
          blueCurrency:
            parsedSettings.blueCurrency !== undefined
              ? parsedSettings.blueCurrency
              : 0,
        };
      }
    } catch (error) {
    ...
    const saveMapPoolSettingsToStorage = () => {
      try {
        ...
        localStorage.setItem(
          "mapPoolSettings",
          JSON.stringify(mapPoolSettings),
        );
  ...
  useEffect(() => {
    setTeams((prev) =>
      prev.map((team) => {
        ...
        if (settings.matchMode === "teamvs") {
          ...
          avatarUrl = isRed
            ? settings.redTeamAvatarUrl
            : settings.blueTeamAvatarUrl;
        } else {
          // 1v1模式：使用玩家名称和玩家头像
    ...
    settings.redTeamName,
    settings.blueTeamName,
    settings.redPlayer,
    settings.bluePlayer,
    settings.matchMode,
    settings.redTeamAvatarUrl,
  ...
  const handleBeatmapRightClick = (beatmap: BeatmapCard) => {
    ...
    if (bannedBy) {
      ...
      // 只删除最新的ban记录，而不是所有记录
      setBanPickState((prev) => {
        const historyForBeatmap = prev.history.filter(
          (record) =>
            record.team === bannedBy &&
            record.action === "ban" &&
        ...
        const latestIndex = prev.history.findIndex(
          (record) =>
    ...
    const fetchMapPoolData = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // 转换为BeatmapCard格式
          const convertedBeatmaps: BeatmapCard[] = approvedMaps.map(
            (map: any) => ({
              id: map.id,
              ...
              coverUrl:
                map.coverUrl ||
                `https://assets.ppy.sh/beatmaps/${map.beatmapsetId}/covers/cover.jpg`,
  ...
  useEffect(() => {
    ...
    const needsUpdate = beatmaps.some((beatmap) => {
      ...
      if (historyForBeatmap.length === 0) {
        return (
          beatmap.status !== "available" ||
          beatmap.bannedBy !== undefined ||
          beatmap.pickedBy !== undefined
        );
      }
      ...
      if (latestRecord.action === "ban") {
        return (
          ...
          beatmap.bannedBy !== latestRecord.team ||
          beatmap.pickedBy !== undefined
        );
      } else if (latestRecord.action === "pick") {
        return (
          beatmap.status !== "picked" ||
          beatmap.pickedBy !== latestRecord.team ||
          beatmap.bannedBy !== undefined
        );
      }
  ...
  useEffect(() => {
    ...
    if (
      ...
      JSON.stringify(refereeState.availableMaps)
    ) {
          ...
          {(!victoryState.isVisible || !victoryState.hideScorePanel) && (
            <>
              ...
              >
                ...
                >
                  {settings.matchInfo} BO{settings.boFormat.slice(2)} (抢
                  {winScore}分)
                </div>
