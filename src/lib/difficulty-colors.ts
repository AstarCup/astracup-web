...
// 查找相邻的两个关键点
const findNearestKeys = (
  starRating: number,
): { lower: number; upper: number } => {
  const keys = Object.keys(DIFFICULTY_COLOR_MAP)
    .map(Number)
    .sort((a, b) => a - b);

  // 边界情况
  if (starRating <= keys[0]) return { lower: keys[0], upper: keys[0] };
  if (starRating >= keys[keys.length - 1])
    return { lower: keys[keys.length - 1], upper: keys[keys.length - 1] };
