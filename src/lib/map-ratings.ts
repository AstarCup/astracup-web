  ...
  try {
    ...
    try {
      ...
      if ((error as any).code !== "ER_CANT_DROP_FIELD_OR_KEY") {
        console.log(
          "Error removing unique constraint:",
    ...
    try {
      await connection.execute(`
            ...
            `);
      console.log(
        "Removed unique_user_rating constraint from map_ratings table",
      );
    } catch (error: any) {
      // 如果约束不存在，忽略错误
      if ((error as any).code !== "ER_CANT_DROP_FIELD_OR_KEY") {
        console.log(
          "Error removing unique_user_rating constraint:",
          (error as any).message,
        );
      }
    }
    ...
    try {
      const [columns] = await connection.execute(
        `SHOW COLUMNS FROM map_ratings LIKE 'avatar_url'`,
      );
      ...
      console.log(
        "Error checking/adding avatar_url column:",
        (alterError as Error).message,
      );
    }
...
export const mapRatingsStorage = {
  ...
  async addRating(
    ...
    try {
      ...
      if (comment && comment.trim()) {
        ...
        const insertCommentResultHeader =
          insertCommentResult as mysql.ResultSetHeader;
  ...
  async getRatingStats(mapSelectionId: number): Promise<RatingStats> {
    try {
      ...
      const averageRating = statsResult[0]?.averageRating
        ? parseFloat(statsResult[0].averageRating)
        : 0;
  ...
  // 批量获取评分统计
  async getBatchRatingStats(
    mapSelectionIds: number[],
  ): Promise<{ [key: number]: RatingStats }> {
    ...
    try {
      ...
      statsResult.forEach((row) => {
        ...
        if (result[id]) {
          result[id].averageRating = row.averageRating
