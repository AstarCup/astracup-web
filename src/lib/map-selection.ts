...
export const initMapSelectionDatabase = async (): Promise<void> => {
  try {
    ...
    try {
      await connection.execute(
        `SELECT modPosition FROM map_selections LIMIT 1`,
      );
    } catch (error: unknown) {
      const mysqlError = error as { code?: string };
      if (mysqlError.code === "ER_BAD_FIELD_ERROR") {
    ...
    try {
      await connection.execute(
        `SELECT selectedByUsername FROM map_selections LIMIT 1`,
      );
    } catch (error: unknown) {
      const mysqlError = error as { code?: string };
      if (mysqlError.code === "ER_BAD_FIELD_ERROR") {
    ...
    try {
      await connection.execute(
        `SELECT selectedByAvatar FROM map_selections LIMIT 1`,
      );
    } catch (error: unknown) {
      const mysqlError = error as { code?: string };
      if (mysqlError.code === "ER_BAD_FIELD_ERROR") {
    ...
    try {
      await connection.execute(
        `SELECT customDTRate FROM map_selections LIMIT 1`,
      );
    } catch (error: unknown) {
      const mysqlError = error as { code?: string };
      if (mysqlError.code === "ER_BAD_FIELD_ERROR") {
    ...
    try {
      await connection.execute(
        `SELECT customModName FROM map_selections LIMIT 1`,
      );
    } catch (error: unknown) {
      const mysqlError = error as { code?: string };
      if (mysqlError.code === "ER_BAD_FIELD_ERROR") {
    ...
    try {
      await connection.execute(
        `SELECT title_unicode FROM map_selections LIMIT 1`,
      );
    } catch (error: unknown) {
      const mysqlError = error as { code?: string };
      if (mysqlError.code === "ER_BAD_FIELD_ERROR") {
    ...
    try {
      await connection.execute(
        `SELECT artist_unicode FROM map_selections LIMIT 1`,
      );
    } catch (error: unknown) {
      const mysqlError = error as { code?: string };
      if (mysqlError.code === "ER_BAD_FIELD_ERROR") {
  ...
  async getMapSelections(
    ...
    try {
      ...
      return (rows as any[]).map((row) => ({
        ...
        customDTRate: row.customDTRate
          ? parseFloat(row.customDTRate)
          : undefined,
  ...
  async addMapSelection(
    ...
    try {
      ...
      const params = [
        ...
        selection.selectedByUsername !== undefined
          ? selection.selectedByUsername
          : null,
        selection.selectedByAvatar !== undefined
