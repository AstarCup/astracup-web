...
export function parseBeatmapUrl(url: string): {
  beatmapId?: number;
  beatmapsetId?: number;
} {
  // 首先检查是否直接输入了数字ID
...
// 获取单个beatmap信息
export async function getBeatmapInfo(
  beatmapId: number,
  ...
  try {
    ...
    return {
      ...
      title_unicode:
        data.beatmapset?.title_unicode || data.beatmapset?.title || "",
      artist: data.beatmapset?.artist || "",
      artist_unicode:
        data.beatmapset?.artist_unicode || data.beatmapset?.artist || "",
      ...
      cover_url:
        data.beatmapset?.covers?.cover || data.beatmapset?.covers?.card || "",
    };
  } catch (error) {
...
// 获取用户头像（带fallback机制）
export async function getUserAvatarWithFallback(
  osuId: string,
  username: string,
): Promise<string> {
  try {
    // 1. 首先尝试从数据库获取
    const { getUser } = await import("@/lib/prisma-registrations");
    ...
    try {
      ...
      console.warn(
        `Failed to get avatar from osu! API for user ${username} (${osuId}):`,
        apiError,
      );
    }
    ...
    console.error(
      `Error getting avatar for user ${username} (${osuId}):`,
      error,
    );
    return "/unknow.svg";
  }
...
export async function getBeatmapsetInfo(
  beatmapsetId: number,
): Promise<BeatmapInfo[]> {
  try {
    // 获取客户端token
