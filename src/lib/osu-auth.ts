...
export function getOsuAuthUrl(): string {
  if (!OSU_CLIENT_ID) {
    console.error(
      "OSU_CLIENT_ID is not configured. Please set the OSU_CLIENT_ID environment variable.",
    );
  ...
  if (!OSU_CLIENT_SECRET) {
    console.warn(
      "OSU_CLIENT_SECRET is not configured. This may cause token exchange to fail.",
    );
  }
...
export async function getOsuToken(code: string): Promise<{
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `Failed to parse token response: ${jsonError instanceof Error ? jsonError.message : String(jsonError)}`,
      );
    }
  } catch (error) {
